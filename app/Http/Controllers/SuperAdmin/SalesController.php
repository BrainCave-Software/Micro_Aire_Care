...
class SalesController extends Controller
{
    ...
    public function addQuotation(Request $request)
    {
        ...
        $allProductDetails1 = $_REQUEST['allProductDetails1'];
        ...
        if ($validator->fails()) {
        ...
        } else if (sizeof(json_decode($allProductDetails1)) === 0) {
            ...
            Quotation::insert([
                ...
                "mobile_no" => $request->cumobileNo,
                "email_id" => $request->cuemailId,
                "customer_type" => $request->cucustomerType,
                "payment_terms" => $request->paymentTerms,
                "products_details" => $allProductDetails1,
    ...
    {
        $id = "Yes";
        return response()->json(Order::all()->where("invoicestatus", "!=", $id));
    }
    // fetch all quotations
    public function getQuotations()
    {
        return response()->json(DB::table('quotations')->join('customers', 'customers.id', '=', 'quotations.customer_name')->select('quotations.*', 'customers.customer_name as cust_name')->paginate(5));
    }
    ...
    {
        $id = $_GET['id'];
        return response()->json(DB::table('orders')->where("id", $id)->get());
    }
    // fatch a single quotation for edit and view
    public function getQuotationDtl()
    {
        $id = $_GET['id'];
        return response()->json(DB::table('quotations')->where("id", $id)->get());
    }

    // fatch order details
    public function getFtOrders()
    {
        $id = $_GET['id'];
    ...
    // get product list to stock
    public function getFilterProductsNam()
    {
        return response()->json(Stock::all('product_name')->distint()->get());
    }
    ...
    {
        return response()->json(Customer::all()->where('customer_type', "business"));
    }

    // fetch customer list
    public function getCustomer()
    {
        return response()->json(Order::all()->where('shipping_type', "dropshipping"));
    }

    // ***************************************************************************************************//
    //                                           ORDERS SECTION                                          //
    //****************************************************************************************************// 
    // Featch customer name 
    public function frtchCustomerNameSaleOreders()
    {
        return response()->json(Customer::all()->where('customer_type', "business"));
    }
    // fatch address
    ...
    {
        $id = $_GET['customer_name'];
        return response()->json(Customer::all()->where('customer_name', $id));
    }
    // add orders
    public function dlts (Request $request)
    {
    ...
    {
        return response()->json(DB::table('orders')->paginate(5));
    }
    ...
    {
        $id = "Yes";
        return response()->json(Quotation::all()->where("order_status", "!=", $id));
    }
    // fetch single customer details
    public function getSaleOrdersDetails()
    {
        $id = $_GET['customer_id'];
        return response()->json(Quotation::all()->where('customer_id', $id));
    }
    // Update orders
    ...
    {
        $status = $_GET['status'];
        return response()->json(Order::all()->where('status', $status));
    }
    // filter invoice
    public function filterOrdersName()
    {
        return response()->json(DB::table('orders')->where('customer_name', 'LIKE', '%' . $_GET['user'] . '%')->paginate(5));
    }

    // Fetch Special Price Filtered Products Detials
    public function getFilterProductsOrders()
    {
        $customer_id = $_GET['id'];
        // $all_products = product::all()->groupBy(['product_name', 'owner_id'])->toArray();
        $all_products = product::all()->unique(['product_name'])->toArray();
    ...
    public function fetchProductsDetialsInfoVer()
    {
    ...
    public function fatchProductDetialsData()
    {
    }
    ...
    {
        $proId = $_GET["order_id"];
        return response()->json(Order::all()->where('order_id', $proId));
    }
    ...
    public function storeInvoice(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json(['error' => $validator->getMessageBag()->toArray()]);
        } else if ($request->orderNumber == "" && $request->invoiceNumber == "") {
            ...
            SalesInvoice::insert([
                ...
                "quotation_no" => $request->orderNumber,
                "Qref_no" => $request->invoiceNumber,
                "customer_name" => $request->customerName,
            ...
            Order::where('order_id', $request->orderNumber)
                ->update([
                    "invoicestatus" => "Yes",
                    "display" => "none"
                ]);
    ...
    {
        $id = $_GET["pro"];
        $data = SalesInvoice::all()->where('Qref_no', $id);
        return response()->json($data);
    }
